...
data class SongEntity(
    ...
    fun toSong() = Song(
        ...
        ffmpegLoudness = ffmpegLoudness,
        ffmpegPeak = ffmpegPeak,
        lyrics = lyrics
    )
}
...
class MusicRepository(context: Context) {
    ...
    suspend fun saveLibrary(songs: List<Song>, playlists: List<Playlist>) {
        ...
        val songsToInsert = songs.map { song ->
            ...
            if (existing != null) {
                entity.copy(
                    ffmpegLoudness = existing.ffmpegLoudness,
                    ffmpegPeak = existing.ffmpegPeak,
                    lyrics = entity.lyrics ?: existing.lyrics
                )
            } else {
                entity
            }
    ...
    // ── 響度相關 ────────────────────────────────
    suspend fun updateFfmpegLoudness(trackId: String, loudness: Double, peak: Double) {
        val song = musicDao.getSongById(trackId) ?: return
        musicDao.updateSong(song.copy(
            ffmpegLoudness = loudness,
            ffmpegPeak = peak
        ))
    }

    suspend fun getSongsWithoutLoudness(): List<Song> {
        return musicDao.getSongsWithoutFfmpegLoudness().map { it.toSong() }
    }

    suspend fun clearAllLoudness() {
        musicDao.clearAllFfmpegLoudness()
    }
}
...
interface MusicDao {
    ...
    // ── 響度相關 ────────────────────────────────
    @Query("SELECT * FROM songs WHERE ffmpegLoudness IS NULL")
    suspend fun getSongsWithoutFfmpegLoudness(): List<SongEntity>

    @Query("UPDATE songs SET ffmpegLoudness = NULL, ffmpegPeak = NULL")
    suspend fun clearAllFfmpegLoudness()
}
