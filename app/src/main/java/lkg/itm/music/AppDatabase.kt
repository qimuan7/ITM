...
interface SongPlayStatDao {
    ...
    @Query("""
        INSERT INTO song_play_stats (trackId, playCount, lastPlayedTime) 
        VALUES (:trackId, 1, :currentTime)
        ON CONFLICT(trackId) DO UPDATE SET 
        playCount = playCount + 1, 
        lastPlayedTime = :currentTime
    """)
    suspend fun incrementPlayCount(trackId: String, currentTime: Long = System.currentTimeMillis())
}
...
class MusicRepository(context: Context) {
    ...
    suspend fun recordPlay(trackId: String) {
        playStatDao.incrementPlayCount(trackId)
    }
